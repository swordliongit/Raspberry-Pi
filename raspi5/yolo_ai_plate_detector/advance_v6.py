...
class LicensePlateReader:
    def __init__(self):
        ...
        self.read_cooldown = 0.05  # Faster updates
        self.reset_threshold = 0.5  # More tolerant to movement

        # Plate locking mechanism
        self.locked_plate = None
        self.lock_time = None
        self.lock_duration = 0.3  # Shorter lock duration
        self.stable_readings = []
        self.max_stable = 2  # Fewer readings needed
    ...
    def should_reset_detection(self, current_box, current_conf):
        ...
        last_size = (self.last_box[2] - self.last_box[0]) * (self.last_box[3] - self.last_box[1])
        
        # Check for dramatic size change (potential new plate)
        size_ratio = current_size / last_size if last_size > 0 else float('inf')
        if size_ratio > 1.5 or size_ratio < 0.67:  # Size changed by more than 50%
            return True
        
        # Check for significant position change (more tolerant)
        position_change = abs(current_center[0] - last_center[0]) + abs(current_center[1] - last_center[1])
        box_size = max(current_box[2] - current_box[0], current_box[3] - current_box[1])
        if position_change > box_size * 0.5:  # Increased threshold
            return True
        
        # Check for significant confidence change (more tolerant)
        if abs(current_conf - self.last_confidence) > 0.2:  # Increased threshold
            return True
